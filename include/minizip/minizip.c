/*
   a bastardized version of the following:
...
*/
    ...
    //{
        for (i=1;i<argc;i++)
        {
            ...
            else if ((*argv[i])=='>') // strip characters from front of string --bhs
            {
              sscanf(argv[i]+1, "%u", &strip);
              // printf("strip: %s (%i) \n",argv[i]+1, strip);
            }
            else
                if (zipfilenamearg == 0)
...
#        endif
        ...
        for (i=zipfilenamearg+1;(i<argc) && (err==ZIP_OK);i++)
        {
                  ...
                  (strlen(argv[i]) == 2)))
            {
                FILE * fin;
                int size_read;
                const char* filenamein = argv[i];
                const char* filenameinzip = argv[i] + strip;
                zip_fileinfo zi;
                ...
                zi.internal_fa = 0;
                zi.external_fa = 0;
                filetime((char *)filenamein,&zi.tmz_date,&zi.dosDate);
...
*/
                if ((password != NULL) && (err==ZIP_OK))
                    err = getFileCrc(filenamein,buf,size_buf,&crcFile);
